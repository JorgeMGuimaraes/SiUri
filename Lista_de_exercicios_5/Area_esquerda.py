...
soma ou a média considerando somente aqueles elementos que estão na área esquerda da matriz, conforme ilustrado abaixo (área verde).
...
def calculaSomaOuMedia(vs, tipo):
    ...
    for i in range(len(vs[0])):
        for j in range(len(vs[0])):
            if abaixoDaDiagonalPrincipal(i, j) and acimaDaDiagonalSecundaria(i, j):
                soma            += vs[i][j]
                divisorDaMedia  += 1
