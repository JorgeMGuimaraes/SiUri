...
a soma ou a média considerando somente aqueles elementos que estão acima da diagonal secundária da matriz, conforme ilustrado 
abaixo (área verde).
...
def calculaSomaOuMedia(vs, tipo):
    ...
    for i in range(len(vs[0])):
        for j in range(len(vs[0])):
            if acimaDaDiagonalSecundaria(i, j):
                soma            += vs[i][j]
                divisorDaMedia  += 1
