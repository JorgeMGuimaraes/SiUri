...
a soma ou a média considerando somente aqueles elementos que estão na área inferior da matriz, conforme ilustrado abaixo 
(área verde).

Entrada

A primeira linha de entrada contem um único caractere Maiúsculo O ('S' ou 'M'), indicando a operação (Soma ou Média) que deverá 
ser realizada com os elementos da matriz. Seguem os 144 valores de ponto flutuante de dupla precisão (double) que compõem a matriz.
...
def calculaSomaOuMedia(vs, tipo):
    ...
    for i in range(len(vs[0])):
        for j in range(len(vs[0])):
            if abaixoDaDiagonalPrincipal(i, j) and abaixoDaDiagonalSecundaria(i, j):
                soma            += vs[i][j]
                divisorDaMedia  += 1
